        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let batch of batches">
              ...
              <td>
                <button class="btn-blue" (click)="viewBatch(batch.id!)">View</button>
                <button class="btn-red" (click)="deleteBatch(batch.id!)">Delete</button>
              </td>
            </tr>
          </tbody>
        </table>
...
export class BatchListComponent implements OnInit {
  ...
  constructor(private portalService: PortalService, private profileService: ProfileService) {}
  ...
  loadBatches(): void {
    this.portalService.getBatches().subscribe({
      ...
      error: (err) => this.logActivity('❌ Failed to fetch batches: ' + err.message)
    });
  }
  ...
  uploadBatch(): void {
    ...
    const formattedTime = `${timestamp.getFullYear()}-${(timestamp.getMonth() + 1)
      .toString().padStart(2, '0')}-${timestamp.getDate()
        .toString().padStart(2, '0')}_${timestamp.getHours()
          .toString().padStart(2, '0')}-${timestamp.getMinutes()
            .toString().padStart(2, '0')}-${timestamp.getSeconds()
    ...
    this.portalService.uploadBatch(renamedFile, this.merchant.merchantId, originalFileName).subscribe({
      next: (res) => {
  ...
  deleteBatch(id: number): void {
    this.portalService.deleteBatch(id).subscribe({
      next: () => {
        this.logActivity(`🗑️ Batch ID ${id} deleted.`);
        this.loadBatches();
      },
      error: (err) => this.logActivity(`❌ Failed to delete batch: ${err.message}`)
    });
  }
